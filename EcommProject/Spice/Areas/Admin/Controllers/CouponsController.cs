    ...
    public class CouponsController : Controller
    {
        ...
        public async Task<IActionResult> Create(Coupon coupon)
        {
            if (ModelState.IsValid)
            {
                ...
                {
                    byte[] picture = null;
  /*Image Fitch*/   var fs = files[0].OpenReadStream(); //حيستقبل الصورة عشان يحولها لداتا بايت وعشان نخزنها بالداتا بيس
  /*Image Store*/   var ms = new MemoryStream();// بعرف الميموري ستريم عشان أستخدمها بعدين لتخزين الصورة فيها
